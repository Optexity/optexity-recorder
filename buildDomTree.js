export function buildDomTree(args) {
  const {
    doHighlightElements,
    focusHighlightIndex,
    viewportExpansion,
  ...
  function getHoverPointerElements() {
    ...
    // Step 1: Collect all selectors that apply `cursor: pointer` on `:hover`
    for (const sheet of document.styleSheets) {
      let rules;
      try {
        rules = sheet.cssRules;
      } catch (e) {
        continue; // Skip stylesheets from different origins (CORS issue)
      }
      ...
      for (const rule of rules) {
        if (
          ...
          pointerSelectors.push(rule.selectorText);
        }
      }
    }

    // Step 2: Use those selectors to find matching elements in DOM
    const elements = new Set();
    for (const selector of pointerSelectors) {
      try {
        document
          .querySelectorAll(selector.replace(":hover", ""))
          .forEach((el) => elements.add(el));
      } catch (e) {
        // Invalid selectors might throw, ignore
      }
    }
  ...
  const hoverPointerEls = getHoverPointerElements();
  ...
  function getXPathTree(element, stopAtBoundary = true) {
    ...
    // Add [1] to segments that don't end with a position index
